...
import { Save, Users, Image as ImageIcon, Eye, BarChart3 } from "lucide-react";
...
export default async function AdminNewsletterPromoEditPage({ params, searchParams }: Props) {
  ...
  const [{ data: newsletter }, { data: banners }, { data: campaign }, boardPosts] = await Promise.all([
    supabase
      .from("newsletters")
      .select("id, title, slug, subject, preheader, thumbnail_url, status, blocks, newsletter_type")
      .eq("id", id)
      .maybeSingle(),
    supabase
    ...
    supabase
      ...
      .select("id, name, send_type, status, scheduled_at, recurring_time, range_start, range_end")
      .eq("newsletter_id", id)
      .order("created_at", { ascending: false })
      .limit(1)
      .maybeSingle(),
  ...
  if (!newsletter || newsletter.newsletter_type !== "PROMOTIONAL") notFound();
  ...
  return (
    <div>
      <PageHeader
        title="홍보 뉴스레터 작성"
        description={`"${newsletter.title}" 홍보 뉴스레터를 수정합니다.`}
        actions={
      ...
      <form action={savePromoNewsletter.bind(null, newsletter.id)} className="space-y-6">
        ...
        <PromoCampaignSection
          ...
          defaultSendType={campaign?.send_type ?? "IMMEDIATE"}
