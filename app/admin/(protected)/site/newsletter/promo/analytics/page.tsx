...
import { UserPlus, Ban, Eye, Send, MailOpen, MousePointerClick } from "lucide-react";
...
// Promotional-newsletter-only counterpart to ../analytics/page.tsx — same
// shape, but scoped to audience='PROSPECTS' campaigns / newsletter_type=
// 'PROMOTIONAL' newsletters / deliveries with a prospect_id, so it never
// mixes in the subscriber-facing newsletter's numbers (and vice versa).
export default async function AdminNewsletterPromoAnalyticsPage() {
  ...
  const [
    ...
    supabase.from("newsletter_prospects").select("*", { count: "exact", head: true }),
    supabase.from("newsletter_suppressions").select("*", { count: "exact", head: true }),
    supabase
      .from("newsletter_campaigns")
      .select("*", { count: "exact", head: true })
      .eq("status", "SENT")
      .eq("audience", "PROSPECTS"),
    supabase
  ...
  const stats = [
    { label: "등록된 대상자", value: `${prospectCount ?? 0}명`, icon: UserPlus },
    { label: "수신거부(공통)", value: `${suppressedCount ?? 0}명`, icon: Ban },
    { label: "발송 완료 캠페인", value: `${sentCampaignCount ?? 0}건`, icon: Send },
    { label: "홍보 뉴스레터 총 조회수", value: `${totalViews.toLocaleString()}회`, icon: Eye },
    {
      label: "평균 오픈율",
  ...
  return (
    <div className="max-w-6xl">
      <PageHeader
        title="홍보 뉴스레터 통계"
        description="구독자를 모으기 위한 홍보 뉴스레터의 발송/열람/클릭 현황입니다. 실제 구독자 발송 통계와는 분리되어 집계됩니다."
        actions={
          <Link
      ...
      <div className="rounded-xl border border-border bg-white p-5">
        <h3 className="font-semibold text-foreground mb-4">조회수 상위 홍보 뉴스레터</h3>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {(topNewsletters ?? []).map((n) => (
              <TableRow key={n.id}>
                ...
                <TableCell className="text-muted-foreground">{(n.view_count ?? 0).toLocaleString()}</TableCell>
                <TableCell className="text-muted-foreground">
